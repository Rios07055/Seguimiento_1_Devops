...
@app.get("/", response_model=list[schemas.FacultadRead])
def read_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_all(db, skip=skip, limit=limit)

@app.post("/", response_model=schemas.FacultadRead, status_code=201)
def create_item(item: schemas.FacultadCreate, db: Session = Depends(get_db)):
    return crud.create(db, item)

@app.get("/{item_id}", response_model=schemas.FacultadRead)
...
@app.put("/{item_id}", response_model=schemas.FacultadRead)
...
@app.delete("/{item_id}", response_model=schemas.FacultadRead)
